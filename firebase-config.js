...
(function () {
  ...
  function resolveLatLng(data, depth) {
    ...
    const topLng = coerceNumber(
      data.lng ?? data.longitude ?? data.lon ?? data.Lng ?? data.Longitude
    );
  ...
  /**
   * Location UI reads Firestore: collection "siteContent", document id "location".
   * Expected fields (all optional except what you use): address, phone, email, hours,
   * lat, lng, mapUrl — same as Firebase Console.
   * "about" + nested about.location fill in only when a field is missing on "location".
   * Coordinates: if "location" exists, lat/lng come only from that doc (not stale about data).
   */
  ...
  window.fetchMergedSiteLocationData = async function () {
    ...
    const nested =
      aboutData.location && typeof aboutData.location === "object" ? aboutData.location : {};
    const locData = locationSnap.exists ? locationSnap.data() : null;

    const merged = locData
      ? { ...aboutData, ...nested, ...locData }
      : { ...aboutData, ...nested };

    let coords = null;
    if (locationSnap.exists && locData) {
      coords = resolveLatLng(locData);
    } else {
      coords = resolveLatLng(merged);
    }
